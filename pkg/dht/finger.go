...
func newFingerTable(node *protov1.Node, m int) fingerTable {
...
type fingerEntry struct {
	...
	Node *protov1.Node // RemoteNode that Start points to
}

// newFingerEntry returns an allocated new finger entry with the attributes set
func newFingerEntry(id []byte, node *protov1.Node) *fingerEntry {
	return &fingerEntry{
		Id:   id,
		Node: node,
	}
...
func (n *Node) fixFinger(next int) int {
	nextHash := fingerID([]byte(n.cnf.Id), next, n.cnf.HashSize)
